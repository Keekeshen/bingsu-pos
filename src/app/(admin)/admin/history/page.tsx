...
export default function SalesHistoryPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    // Only show paid orders (served = paid, awaiting delivery; completed = delivered)
    if (statusFilter === "all") {
  ...
  function openReceipt(order: Order) {
    ...
    const rounding = isTable ? +(order.total_amount - (order.subtotal - order.discount_amount) - svc).toFixed(2) : 0;
    ...
    if (isTable) {
      ...
      // POS order — work backwards to get service charge from total_amount
      // total = subtotal - discount - tierDiscount + serviceCharge
      // We store total_amount and subtotal, discount_amount covers all discounts
      const posServiceCharge = +(order.total_amount - (order.subtotal - order.discount_amount)).toFixed(2);
      setReceipt({
        order: receiptOrder,
        items,
        customerName:  order.customer_name ?? undefined,
    ...
    <div className="flex flex-col gap-5 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-400">
            {filtered.length} order{filtered.length !== 1 ? "s" : ""} · RM {totalRevenue.toFixed(2)} revenue
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input placeholder="Search order #, customer, table, voucher…"
      ...
      {loading ? (
        <div className="space-y-2">
          {[...Array(6)].map((_, i) => <div key={i} className="h-16 animate-pulse rounded-xl bg-zinc-100" />)}
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map(order => {
            ...
            const isTable = order.source === "table";
            return (
              <div key={order.id} className="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
                {/* Row header */}
                <button
                ...
                >
                  ...
                  {order.status === "completed" ? (
                    <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 shrink-0" title="Delivered" />
                  ) : (
                    <Clock className="h-3.5 w-3.5 text-amber-400 shrink-0" title="Paid · pending delivery" />
                  )}

                  {/* Order # */}
                  <span className="font-mono text-sm font-semibold text-zinc-800 min-w-[110px]">{order.order_number}</span>

                  {/* Customer */}
                  <span className="text-sm text-zinc-500 flex-1 truncate">
                    {order.customer_name ?? <span className="italic text-zinc-400">Walk-in</span>}
                  </span>
                  ...
                  {order.payment_method && (
                    <span className="flex items-center gap-1 text-xs text-zinc-500 shrink-0">
                      ...
                      <span className="hidden sm:inline">{PAYMENT_LABELS[order.payment_method] ?? order.payment_method}</span>
                    </span>
                  )}

                  {/* Total */}
                  <span className="text-sm font-bold tabular-nums text-zinc-900 shrink-0">
                ...
                {isOpen && (
                  <div className="border-t border-zinc-100 bg-zinc-50 px-4 py-3">
                    ...
                    {/* Totals breakdown */}
                    <div className="space-y-0.5 border-t border-dashed border-zinc-200 pt-2 mb-3 text-sm">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 border-t border-zinc-200 pt-2">
                      <div className="text-xs text-zinc-400 space-y-0.5">
                        ...
                        {order.points_earned > 0 && (
                          <p className="text-emerald-600 font-medium">+{order.points_earned} pts earned</p>
                        )}
                        {order.status === "served" && (
                          <p className="text-amber-500 font-medium flex items-center gap-1">
                            <Clock className="h-3 w-3" /> Paid · pending food delivery
                          </p>
                        )}
                      </div>
