  ...
  return function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ value?: number }>; label?: string }) {
    ...
    return (
      <div className="rounded-lg border border-zinc-200 bg-white px-3 py-2 shadow-md text-xs">
        <p className="font-medium text-zinc-700">{fmt(label as string)}</p>
...
export default function RevenueLineChart({ data, xFormat }: Props) {
  ...
  const tickIndices = showAll
    ...
    : new Set(data.map((_, i) => i).filter((i) => i % 7 === 0 || i === data.length - 1));
    ...
    <ResponsiveContainer width="100%" height={260}>
      <LineChart data={data} margin={{ top: 8, right: 16, bottom: 0, left: 0 }}>
        ...
        <XAxis dataKey="date" tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: "#a1a1aa" }}
          tickFormatter={(v, i) => tickIndices.has(i) ? fmt(v as string) : ""} />
        <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: "#a1a1aa" }}
          tickFormatter={(v: number) => `RM ${v >= 1000 ? `${(v / 1000).toFixed(1)}k` : v}`} width={64} />
        <Tooltip content={<TooltipContent />} />
        <Line type="monotone" dataKey="revenue" stroke="#18181b" strokeWidth={2}
          dot={showAll ? { r: 3, fill: "#18181b", strokeWidth: 0 } : false}
